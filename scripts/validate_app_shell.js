...
assert.ok(app.includes('assets/reading-art-20260830.js?v=3'), 'Result illustration engine is not loaded');
...
assert.ok(wrapper.includes('english-app7.html?v=20260831-clean-canva-art1'), 'Wrapper cache key was not updated');
for (const file of ['blogger.html', 'blogger-english.html']) {
  assert.ok(read(file).includes('english.html?v=blogger-clean-canva-art-20260831'), file + ' cache key was not updated');
}
assert.ok(read('blogger-post-final.html').includes('english.html?v=20260831-clean-canva-art1'), 'Blogger post cache key was not updated');
