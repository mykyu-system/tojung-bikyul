...
assert.ok(app.includes('assets/reading-art-20260830.js?v=1'), 'Result illustration engine is not loaded');
...
assert.ok(wrapper.includes('english-app7.html?v=20260830-reading-art1'), 'Wrapper cache key was not updated');
for (const file of ['blogger.html', 'blogger-english.html']) {
  assert.ok(read(file).includes('english.html?v=blogger-reading-art-20260830'), file + ' cache key was not updated');
}
assert.ok(read('blogger-post-final.html').includes('english.html?v=20260830-reading-art1'), 'Blogger post cache key was not updated');
